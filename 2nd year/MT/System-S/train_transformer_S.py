...
df_train = pd.read_csv('/home/anca/Fac/MT/System-S/train_new.csv')
df_train = df_train.loc[df_train['label'].isin([0,1])]
df_train['label'] = df_train['label'].apply(lambda x: int(x))

train_texts, val_texts, train_labels, val_labels = train_test_split(
    df_train['tweet'].values.tolist(), df_train['label'].values.tolist(),
...
model = AutoModelForSequenceClassification.from_pretrained(model_name, num_labels=2, ignore_mismatched_sizes=True, hidden_dropout_prob=0.3)
model.to(device)
...
def train(model, train_loader, val_loader, optimizer,scheduler):  
  ...
  for epoch in range(EPOCHS):
    ...
    if best_acc <= val_acc:
        best_acc = val_acc
        checkpoint_path = '/home/anca/Fac/MT/System-S/checkpoints'
